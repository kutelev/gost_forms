...
int drawRect(FILE* file, const double x, const double y, const double width,
    const double height, const bool bold)
{
    fprintf(file, "        <rect x=\"%.6lf\" y=\"%.6lf\" width=\"%.6lf\" "
        "height=\"%.6lf\" fill=\"none\" stroke=\"black\" "
        "stroke-width=\"%s\" />\n", x, y, width, height, bold ? "0.8" : "0.3");

    return 0;
}

int drawLine(FILE* file, const double x1, const double y1, const double x2,
    const double y2, const bool bold)
{
    fprintf(file, "        <line x1=\"%.6lf\" y1=\"%.6lf\" x2=\"%.6lf\" "
...
int drawText(FILE* file, const double x, const double y, const double font_size,
    const char*const text, const Alignment alignment,
    const int angle)
{
    ...
    switch (angle) {
        ...
        case -90:
            fprintf(file, "        <text x=\"%.6lf\" y=\"%.6lf\" "
                ...
                -y, x + (font_size / k2) / 2.0, textStyle(alignment),
                font_size / k1,
                0.0, 0.0, text);
            break;
        case -180:
...
int drawMultilineText(FILE* file, double x, double y, double font_size,
    const char* multiline_text, Alignment alignment, int angle)
{
    int num;
    double line_skip;
    double x_step = 0.0;
    ...
    for(int i = 0; i < num; i++,x+=x_step,y+=y_step) {
        if(columns[i][0] != '\0')KXSvg::drawText(file, x, y, font_size,
            columns[i], KXSvg::Center, angle);
    }
...
int Page::begin(FILE* file) const
{
    ...
    fprintf(file, "<svg width=\"%.6lfmm\" height=\"%.6lfmm\" "
